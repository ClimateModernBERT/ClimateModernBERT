...
def generate_synthetic_data(model, tokenizer, seed_text, style, num_samples, args):
    ...
    for i in range(num_samples):
        messages = [
            {"role": "system", "content": "You are an expert climate journalist and researcher capable of generating high-quality, factual content about climate change based on real news excerpts."},
...
def main():
    ...
    for seed_idx, (idx, row) in enumerate(tqdm(seed_articles.iterrows(), total=len(seed_articles), desc="Processing seeds")):
        ...
        print(f"\n{'='*80}")
        print(f"Seed #{seed_idx + 1}/{num_seeds} (ID: {seed_id})")
        ...
        for style in styles_to_use:
            ...
            try:
                synthetic_samples = generate_synthetic_data(
                    model, tokenizer, seed_text, style, 
                    args.samples_per_seed, args
                )
                ...
                for sample_idx, sample in enumerate(synthetic_samples):
                    ...
                    synthetic_id += 1
                    print(f"    ✓ Generated sample {sample_idx + 1}/{args.samples_per_seed} (ID: {synthetic_id - 1})")
